...
public class TestRunner {
    ...
    public static void run(Class<?> testSuiteClass) throws InvocationTargetException, IllegalAccessException {
        ...
        List<Method> beforeSuiteMethod = priorityToMethodListMap.get(11);
        if (beforeSuiteMethod != null && !beforeSuiteMethod.isEmpty()) {
            beforeSuiteMethod.get(0).invoke(null);
        }
        for (int i = 10; i >= 1; i--) {
        ...
        List<Method> afterSuiteMethod = priorityToMethodListMap.get(0);
        if (afterSuiteMethod != null && !afterSuiteMethod.isEmpty()) {
            afterSuiteMethod.get(0).invoke(null);
        }
        System.out.println("\nВЫПОЛНЕНИЕ ТЕСТОВ: КОНЕЦ\n");

        System.out.println("Количество методов с аннотацией @Test класса " + testSuiteClass.getSimpleName() + ": " + classMethodsCount);
